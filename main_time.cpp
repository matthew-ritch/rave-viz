...
void process_frames(VideoBuffer* vb, double* filt, int kernel_size) {
    ...
    // Process frames in parallel using C++ threads
    parallel_for(indices.begin(), indices.end(), [&](int i) {
        for (int k = 0; k < N_REPS; k++) {
            // Process each frame with temporal convolution
            cv::Mat& delta = deltas[i];
            delta = cv::Scalar(0, 0, 0); // Reset delta
            convolve_through_time(&vb->frames, &delta, filt, kernel_size);
